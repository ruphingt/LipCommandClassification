"""
This class was obtained from https://github.com/jphdotam/keras_generator_example
This is a data generator for video datas
"""
import glob
import random
import os
...
class VideoGenerator:
    ...
    def __init__(self,
        ...
        self.classname_by_id = {i: cls for i, cls in
                                enumerate(sorted({os.path.basename(os.path.dirname(file)) for file in self.filenames_train}))}
    ...
    def generate(self, train_or_test, rotation_range=None, heigt_shift_range=None, width_shift_range=None,
        ...
        while True:
            ...
            for i in range(n_batches):
                ...
                print('filenames_batch', filenames_batch,'x shape', x.shape, 'y.shape', y.shape, 'y', y)
                yield x, y
    ...
    def __generate_data_frome_batch_file_names(self, filenames_batch):
        ...
        for i, filename in enumerate(filenames_batch):
            ...
            # Horizontally augment data
#            if self.horizontal_flip:
#                # T x H x W x C
#                # Flip along the W dimension, which is the horizontal direction
#                npy_flip = np.flip(npy,2)
#                data.append(npy_flip)
#                labels.append(self.id_by_classname[label])
